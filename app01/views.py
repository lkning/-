...
def bind(request):
    ...
    :return: 
    """
    obj = models.UserInfo.objects.filter(id=request.session['user_info']['id']).first()
    return render(request, 'bind.html',{'obj':obj})
...
def bind_qcode(request):
    ...
    try:
        ...
        access_url = access_url.format(
            ...
            state=request.session['user_info']['uid'] # 当前登录用户uid 6cfbec608383fd05c271de92010d455f
        )
        ret['data'] = access_url
...
def show_data(request):
    ...
    return render(request,'show_data.html',{'users':users})